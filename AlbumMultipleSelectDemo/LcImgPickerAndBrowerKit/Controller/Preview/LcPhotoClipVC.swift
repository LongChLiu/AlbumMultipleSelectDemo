...
class LcPhotoClipVC: LcBaseVC,UIScrollViewDelegate {

    var clipBounds: CGSize = CGSize(width: kScreenWidth, height: kScreenWidth) {
            didSet {
                self.photoImageScrollView.frame = CGRect(origin: CGPoint(x: (kScreenWidth-clipBounds.width)/2, y: (kScreenHeight-clipBounds.height)/2), size: clipBounds)
            }
        }
        ...
        private lazy var photoImageScrollView: UIScrollView = {
            ...
            var frame = CGRect(origin: CGPoint(x: 0, y: (kScreenHeight-kScreenWidth)/2), size: self.clipBounds)
        ...
        private lazy var maskView: LcPhotoClipMaskView = {
            let view = LcPhotoClipMaskView(frame: self.view.frame, clipBounds: self.clipBounds)
            return view
        }()
        ...
        var photoImage: UIImage? {
            didSet {
                ...
                setContent = true
                let imageHeight = kScreenWidth*size.height/size.width
                self.photoImageView.frame = CGRect(x: 0, y: 0, width: kScreenWidth, height: imageHeight)
                ...
                if kScreenWidth > clipBounds.width {
                    self.photoImageScrollView.contentOffset.x = (kScreenWidth-clipBounds.width)/2
                } else {
    ...
    //        UIApplication.shared.setStatusBarStyle(.default, animated: true)
        ...
        func scrollViewDidZoom(_ scrollView: UIScrollView) {
            var xcenter = scrollView.center.x-(kScreenWidth-clipBounds.width)/2 , ycenter = scrollView.center.y-(kScreenHeight-clipBounds.height)/2
    ...
    fileprivate class LcPhotoClipMaskView: UIView {
    ...
    convenience override init(frame: CGRect) {
        self.init(frame: frame, clipBounds: CGSize(width: kScreenWidth, height: kScreenWidth))
    }
    ...
    init(frame: CGRect, clipBounds: CGSize) {
        ...
        path.append(UIBezierPath(rect: CGRect(origin: CGPoint(x: (kScreenWidth-clipBounds.width)/2, y: (kScreenHeight-clipBounds.height)/2), size: clipBounds)).reversing())
