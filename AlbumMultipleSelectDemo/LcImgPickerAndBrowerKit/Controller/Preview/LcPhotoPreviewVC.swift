...
class LcPhotoPreviewVC: LcBaseVC, UICollectionViewDelegate, UICollectionViewDataSource ,UICollectionViewDelegateFlowLayout {
    ...
    //  数据源(预览选择时用)
    var photoData = LcPhotoDataSource()
    //  浏览数据源
    ...
    private var willDisplayCellAndIndex: (cell: LcPreviewCollectionViewCell, indexPath: IndexPath)?
    ...
    private lazy var thumbnailCollectionView: UICollectionView = {
        ...
        //  collectionView
        let collectionView = UICollectionView(frame: CGRect(x: 0, y: kScreenHeight-kHomeBarHeight-44 - 81, width: kScreenWidth, height: 80), collectionViewLayout: flowLayout)
    ...
    private lazy var bottomView = LcAlbumBottomView(type: .noPreview)
    ...
    override func didMove(toParent parent: UIViewController?) {
        if parent == nil {
            for id in self.requestIDs {
                LcCacheImgManager.default().cancelImageRequest(id)
            }
        }
    }
        ...
        if let index = self.photoData.assetArray.index(of: self.previewPhotoArray[currentIndex]) {
            ...
            if let selectIndex = self.photoData.seletedAssetArray.index(of: self.previewPhotoArray[currentIndex]){
                if selectStyle == .number{
                    image = UIImage.lcCreateImageWithView(view: LcPhotoNavigationVC.lcGetSelectNuberView(index: "\(selectIndex + 1)"))
                }else{
                    image = LcSelectSkinImage
                }
            }
    ...
    private func setPreviewImage(cell: LcPreviewCollectionViewCell, asset: PHAsset) {
        let pixelScale = CGFloat(asset.pixelWidth)/CGFloat(asset.pixelHeight)
        let id = LcCacheImgManager.default().requestPreviewImage(for: asset, progressHandler: { (progress: Double, error: Error?, pointer: UnsafeMutablePointer<ObjCBool>, dictionry: Dictionary?) in
            //下载进度
            DispatchQueue.main.async {
                let progressView = LcProgressView.showLcProgressView(in: cell.contentView, frame: CGRect(x: cell.frame.width-20-12, y: cell.frame.midY+(cell.frame.width/pixelScale-20)/2-12, width: 20, height: 20))
    ...
    override func rightButtonClick(button: UIButton) {
        ...
        if let index = self.photoData.assetArray.index(of: self.previewPhotoArray[currentIndex]) {
            ...
            let selectIndex = self.photoData.seletedAssetArray.firstIndex(of: self.previewPhotoArray[currentIndex])
            if selectIndex != nil{
                self.leftButton.asyncSetImage(UIImage.lcCreateImageWithView(view: LcPhotoNavigationVC.lcGetSelectNuberView(index: "\(selectIndex! + 1)")), for: .selected)
            }
            
            if selectIndex != nil{
                if self.leftButton.isSelected == false {
                    let indexPath = IndexPath.init(row: selectIndex!, section: 0)
                    self.photoData.seletedAssetArray.remove(at: self.photoData.seletedAssetArray.firstIndex(of: self.previewPhotoArray[currentIndex])!)
    ...
    func thumbnailCollectionViewCellToggeleSelect(indexPath:IndexPath){
        ...
        cell?.contentView.layer.borderColor = LcPhotoAlbumSkinColor.cgColor
        ...
        if collectionView == thumbnailCollectionView {
            ...
            if isFristLoadCell == false{
                if let selectIndex = self.photoData.seletedAssetArray.index(of: self.previewPhotoArray[currentIndex]),selectIndex == indexPath.row{
                    ...
                    smallCell.contentView.layer.borderColor = LcPhotoAlbumSkinColor.cgColor
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if  collectionView == thumbnailCollectionView {
            ...
            let id = LcCacheImgManager.default().requestThumbnailImage(for: asset) { (image: UIImage?, dictionry: Dictionary?) in
                (cell as! LcPreviewCollectionViewCell).photoImage = image ?? UIImage()
            }
            self.requestIDs.append(id)
            
            self.willDisplayCellAndIndex = (cell as! LcPreviewCollectionViewCell, indexPath)
            if indexPath.row == self.currentIndex && self.isFirstCell {
                self.isFirstCell = false
                self.setPreviewImage(cell: cell as! LcPreviewCollectionViewCell, asset: asset)
            }
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if  collectionView == thumbnailCollectionView {
            ...
            return CGSize(width: kScreenWidth+10, height: kScreenHeight)
        }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView == photoCollectionView{
            ...
            if let selectIndex = self.photoData.seletedAssetArray.firstIndex(of: self.previewPhotoArray[currentIndex]){
                self.leftButton.asyncSetImage(UIImage.lcCreateImageWithView(view: LcPhotoNavigationVC.lcGetSelectNuberView(index: "\(selectIndex + 1)")), for: .selected)
                self.leftButton.isSelected = true
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        if scrollView == photoCollectionView{
            if scrollView.contentOffset.x != self.scrollDistance {
                let currentCell = self.photoCollectionView.cellForItem(at: IndexPath(item: self.currentIndex, section: 0)) as! LcPreviewCollectionViewCell
