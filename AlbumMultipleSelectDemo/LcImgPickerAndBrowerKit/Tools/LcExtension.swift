...
extension UIImage {
    class func lcImageFromeBundle(named: String) -> UIImage? {
        if let cacheImageData = LcCacheImgManager.default().getImageMemoryCache(key: named) {
            return UIImage(data: cacheImageData, scale: UIScreen.main.scale)
        }
        let pathName = "/Frameworks/LCImagePickerAndBrower.framework/\(named)"
        if let fullImagePath = Bundle.main.resourcePath?.appending(pathName) {
            ...
            if let imageData = image.pngData() {
                LcCacheImgManager.default().setImageMemoryCache(key: named, data: imageData)
            }
            return image
        }
        return nil
    ...
    class func lcCreateImageWithColor(color: UIColor, size: CGSize) -> UIImage? {
    ...
    class func lcCreateImageWithView(view: UIView) -> UIImage? {
...
extension UIImage {
    func lcSetRoundedCorner(radius: CGFloat) -> UIImage? {
...
extension UIImageView {
    func asyncSetImage(_ image:UIImage?){
        DispatchQueue.global(qos: .userInteractive).async {
            let decodeImage = image?.lcDecodedImage()
    ...
    func setWebImage(url:String?, defaultImage:UIImage?, isCache:Bool, downloadSuccess: ((_ image: UIImage?) -> Void)?) {
        ...
        if isCache {
            var data: Data? = LcCacheImgManager.default().readCacheFromUrl(url: url!)
            if data != nil {
                lcImage = UIImage(data: data!)
                self.asyncSetImage(lcImage)
                if downloadSuccess != nil {
                    downloadSuccess!(lcImage)
                }
            }else{
                ...
                dispath.async(execute: { () -> Void in
                    do {
                        ...
                        if data != nil {
                            lcImage = UIImage(data: data!)
                            //写缓存
                            LcCacheImgManager.default().writeCacheToUrl(url: url!, data: data!)
