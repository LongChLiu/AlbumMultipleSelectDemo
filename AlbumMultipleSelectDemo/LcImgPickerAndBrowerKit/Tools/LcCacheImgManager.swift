...
public class LcCacheImgManager: PHCachingImageManager {
    ...
    private static let shared = LcCacheImgManager()
    
    public override class func `default`() -> LcCacheImgManager {
        return shared
    }
    ...
    func readCacheFromUrl(url: String) -> Data? {
        ...
        if data == nil {
            let path: String = LcCacheImgManager.default().getFullCachePathFromUrl(url: url)
    ...
    func getFullCachePathFromUrl(url: String) -> String {
        var chchePath = NSHomeDirectory().appending("/Library/Caches/LCImageCache")
        ...
        //进行字符串处理
        var newURL: String
        newURL = LcCacheImgManager.default().stringToString(str: url)
    ...
    public func removeAllCache(){
        ...
        let chchePath = NSHomeDirectory().appending("/Library/Caches/LCImageCache")
        let fileManager: FileManager = FileManager.default
        if fileManager.fileExists(atPath: chchePath) {
            do {
